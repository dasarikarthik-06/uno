...
export default function GameTable({ 
  ...
  if (!gameState || !gameState.players || gameState.players.length === 0) {
    return <div className="w-full h-full flex items-center justify-center text-white">Loading game...</div>;
  }

  // Find current player index
  const currentPlayerIndex = gameState.players.findIndex(p => p.userId === currentUserId);
  const currentTurnPlayerName = gameState.players[gameState.currentTurn]?.username || 'Player';
  ...
  const discardCard = gameState.discard?.[gameState.discard.length - 1] || { color: 'RED', value: 5 };
  ...
  return (
    <div 
      ...
      style={{
        background: 'linear-gradient(135deg, #0EA5E9 0%, #06B6D4 25%, #10B981 50%, #FBBF24 75%, #0EA5E9 100%)',
        overflow: 'hidden',
      }}
    >
      {/* Top Info Bar */}
      <div className="flex justify-between items-center px-6 py-3 z-50">
        ...
        <div className="text-white font-bold text-center">
          <div className={`text-lg ${isMyTurn ? 'text-lime-300' : 'text-white'}`}>
      ...
      <div className="flex-1 relative flex items-center justify-center overflow-hidden">
        ...
        {/* Table container - fixed size to prevent overflow */}
        <div 
          className="relative"
          style={{
            width: '90vw',
        ...
        >
          ...
          <div
            ...
            style={{
              background: 'radial-gradient(circle at 50% 50%, #84CC16 0%, #65A30D 100%)',
              boxShadow: '0 30px 60px rgba(0,0,0,0.3), inset 0 2px 4px rgba(255,255,255,0.2)',
              zIndex: 1,
            }}
          />
          ...
          <div className="absolute inset-0" style={{ zIndex: 10 }}>
            ...
            {arrangedPlayers.map((player) => (
              <PlayerSeat
                ...
                isActive={gameState.currentTurn === gameState.players.findIndex(p => p.userId === player.userId)}
